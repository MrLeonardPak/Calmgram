...
namespace calmgram::api_client::use_case {
    ...
    public:
        ...
        int CreateChat(std::vector<int> target_ids) override;  // создание чата с пользователем с target_id 
        void SendMessage(std::string str, int chat_id) override;  // отправка сообщения на сервер
